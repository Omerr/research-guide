...
TOC = --toc --toc-depth 3
METADATA_ARGS = --metadata-file $(METADATA) --highlight-style zenburn.theme
...
EISVOGEL_ARGS = --template eisvogel.latex --pdf-engine xelatex -V block-headings
...
EISVOGEL_DEPENDENCIES = $(PDF_DEPENDENCIES)
...
eisvogel:	$(BUILD)/eisvogel/$(OUTPUT_FILENAME).pdf
...
$(BUILD)/eisvogel/$(OUTPUT_FILENAME).pdf:	$(EISVOGEL_DEPENDENCIES)
	$(ECHO_BUILDING)
	$(MKDIR_CMD) $(BUILD)/eisvogel
	$(CONTENT) | $(CONTENT_FILTERS) | $(PANDOC_COMMAND) $(ARGS) $(EISVOGEL_ARGS) -o $@
#	pdftk frontmatter.pdf $(BUILD)/eisvogel/$(OUTPUT_FILENAME).pdf cat output $(BUILD)/eisvogel/$(OUTPUT_FILENAME)_frontmatter.pdf
	pdftk frontmatter_hardcover.pdf $(BUILD)/eisvogel/$(OUTPUT_FILENAME).pdf cat output $(BUILD)/eisvogel/$(OUTPUT_FILENAME)_frontmatter_hardcover.pdf
	rm $(BUILD)/eisvogel/$(OUTPUT_FILENAME).pdf
	$(ECHO_BUILT)
